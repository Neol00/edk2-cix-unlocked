...
typedef void (*OptProcessFunc)(char *, char *);

typedef struct _Opt {
			char *option;
			int  arg;
			OptProcessFunc process;
			char *descr;
		} Opt;
...
Opt options[] = {
    { "-CC", 0, (OptProcessFunc) pCC,	"Generate C++ output (default=FALSE)"},
    { "-ck", 1, (OptProcessFunc) pCk,	"Set compressed lookahead depth; fast approximate lookahead"},
    { "-cr", 0, (OptProcessFunc) pCr,	"Generate cross reference (default=FALSE)"},
    { "-e1", 0, (OptProcessFunc) pE1,	"Ambiguities/errors shown in low detail (default)"},
    { "-e2", 0, (OptProcessFunc) pE2,	"Ambiguities/errors shown in more detail"},
    { "-e3", 0, (OptProcessFunc) pE3,
    	"Ambiguities for k>1 grammars shown with exact tuples (not lookahead sets)"},
    { "-f",  1, (OptProcessFunc) pFileList,"Read names of grammar files from specified file"}, /* MR14 */
    { "-fe", 1, (OptProcessFunc) pFe,	"Rename err.c"},
    { "-fh", 1, (OptProcessFunc) pFHdr,	"Rename stdpccts.h header (turns on -gh)"},
    { "-fl", 1, (OptProcessFunc) pFl,	"Rename lexical output--parser.dlg"},
    { "-fm", 1, (OptProcessFunc) pFm,	"Rename mode.h"},
    { "-fr", 1, (OptProcessFunc) pFr,	"Rename remap.h"},
    { "-ft", 1, (OptProcessFunc) pFt,	"Rename tokens.h"},
    { "-ga", 0, (OptProcessFunc) pANSI,	"Generate ANSI-compatible code (default=FALSE)"},
    { "-gc", 0, (OptProcessFunc) pCGen,	"Do not generate output parser code (default=FALSE)"},
    { "-gd", 0, (OptProcessFunc) pTGen,	"Generate code to trace rule invocation (default=FALSE)"},
    { "-ge", 0, (OptProcessFunc) pEGen,	"Generate an error class for each non-terminal (default=FALSE)"},
    { "-gh", 0, (OptProcessFunc) pGHdr,	"Generate stdpccts.h for non-ANTLR-generated-files to include"},
    { "-gk", 0, (OptProcessFunc) pDL,	"Generate parsers that delay lookahead fetches until needed"},
    { "-gl", 0, (OptProcessFunc) pLI,	"Generate line info about grammar actions in parser"},
    { "-glms", 0, (OptProcessFunc) pLIms,"Like -gl but replace '\\' with '/' in #line filenames for MS C/C++ systems"},
    { "-gp", 1, (OptProcessFunc) pPre,	"Prefix all generated rule functions with a string"},
    { "-gs", 0, (OptProcessFunc) pSGen,	"Do not generate sets for token expression lists (default=FALSE)"},
    { "-gt", 0, (OptProcessFunc) pAst,	"Generate code for Abstract-Syntax-Trees (default=FALSE)"},
    { "-gx", 0, (OptProcessFunc) pLGen,	"Do not generate lexical (dlg-related) files (default=FALSE)"},
    { "-gxt",0, (OptProcessFunc) pXTGen,	"Do not generate tokens.h (default=FALSE)"},
    { "-k",  1, (OptProcessFunc) pLLK,	"Set full LL(k) lookahead depth (default==1)"},
    { "-o",  1, (OptProcessFunc) pOut,	OutputDirectoryOption},
    { "-p",  0, (OptProcessFunc) pPrt,	"Print out the grammar w/o actions (default=no)"},
    { "-pa", 0, (OptProcessFunc) pPrtA,	"Print out the grammar w/o actions & w/FIRST sets (default=no)"},
    { "-pr",0, (OptProcessFunc) pPred,	"no longer used; predicates employed if present"},
    { "-prc", 1, (OptProcessFunc) pPredCtx,"Turn on/off computation of context for hoisted predicates"},
   	{ "-rl", 1, (OptProcessFunc) pTRes,	"Limit max # of tree nodes used by grammar analysis"},
    { "-stdout",0,  (OptProcessFunc) pStdout,"Send grammar.c/grammar.cpp to stdout"},           /* MR6 */
	{ "-tab", 1, (OptProcessFunc) pTab,	"Width of tabs (1 to 8) for grammar.c/grammar.cpp files"}, /* MR6 */
	{ "-w1", 0, (OptProcessFunc) pW1,	"Set the warning level to 1 (default)"},
	{ "-w2", 0, (OptProcessFunc) pW2,	"Ambiguities yield warnings even if predicates or (...)? block"},
	{ "-",   0, (OptProcessFunc) pStdin,	"Read grammar from stdin" },
    { "-mrhoist",1, (OptProcessFunc) pMRhoist,                                                  /* MR9 */
                                        "Turn on/off k=1 Maintenance Release style hoisting"},  /* MR9 */
    { "-mrhoistk",1, (OptProcessFunc) pMRhoistk,                                                  /* MR9 */
                                        "Turn on/off EXPERIMENTAL k>1 Maintenance Release style hoisting"},  /* MR13 */
    { "-aa"  , 1, (OptProcessFunc) pAA,  "Ambiguity aid for a rule (rule name or line number)"},          /* MR11 */
    { "-aam" , 0, (OptProcessFunc) pAAm,
                                         "Lookahead token may appear multiple times in -aa listing"},    /* MR11 */
    { "-aad" , 1, (OptProcessFunc) pAAd,
                                         "Limits exp growth of -aa listing - default=1 (max=ck value)"}, /* MR11 */
	{ "-info", 1, (OptProcessFunc) pInfo,
      "Extra info: p=pred t=tnodes f=first/follow m=monitor o=orphans 0=noop"},                          /* MR12 */
    { "-treport",1,(OptProcessFunc) pTreport,
                        "Report when tnode usage exceeds value during ambiguity resolution"},            /* MR11 */
	{ "-newAST", 0, (OptProcessFunc) pNewAST,
                 "In C++ mode use \"newAST(...)\" rather than \"new AST(...)\""},                        /* MR13 */
	{ "-tmake", 0, (OptProcessFunc) ptmakeInParser,
                 "In C++ mode use parser's tmake method rather than \"ASTBase::tmake(...)\""},			 /* MR23 */
    { "-alpha",0,(OptProcessFunc) pAlpha,
                 "Provide additional information for \"(alpha)? beta\" error messages"},                 /* MR14 */
    { "-mrblkerr",0,(OptProcessFunc) pMR_BlkErr,                                        /* MR21 */
                 "EXPERIMENTAL change to (...)* and (...)+ syntax error sets"},        /* MR21 */
	{ "-nopurify",0,(OptProcessFunc) pNOPURIFY,
		"Don't use the notorious PURIFY macro (replaced by MR23 initial value syntax) to zero return arguments of rules"},   /* MR23 */
    { "*",   0, (OptProcessFunc) pFile, 	"" },	/* anything else is a file */
	{ NULL,  0, NULL }
 };
