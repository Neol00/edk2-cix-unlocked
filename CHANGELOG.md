...
#### PCIe
- **Added a resizable PCIe 64-bit MMIO window** — new "PCIe 64-bit Window" option at the top of the PCIe Configuration menu. The stock CIX layout gives every root bridge exactly 16 GB of above-4G MMIO, 16 GB aligned (`PCI_HB0RB*_MEM_ABOVE_4G_SIZE` in `PlatformPcieLib.h`). A Resizable BAR has to be placed at an address aligned to its own size, so a GPU asking for a 16 GB BAR can only land on the window's own base address — leaving nothing for the card's remaining BARs or the root port's bridge window, and the resize fails. This is why cards with 16 GB of VRAM did not work, and why a 24 GB card (which asks for a 32 GB BAR) could not fit at all. The window size for the x8 slot is now selectable: `16G` (stock), `32G` (for 16 GB cards), `64G` (for 24 GB cards), `128G` (experimental), or `Auto`. Changing it requires a reboot.
- **Auto sizes the window from the card that is actually installed** — with `Auto` selected, `PcieWindowLib` walks each root port's endpoint over ECAM, reads its PCI Express Resizable BAR capability (extended capability 0x0015), takes the largest size the device advertises support for, and picks the smallest layout that can place that BAR with at least 512 MB of the window left over. It never selects the experimental 128G layout. If no card behind a port advertises Resizable BAR, that port keeps the stock 16 GB. The probe is possible at this point because `PcieInitDxe` has already assigned bus numbers, so endpoints are reachable through ECAM before PciBusDxe enumerates.
- **Every layout keeps the root complexes at their fixed hardware base addresses** — only the x8 slot's window grows, and only upward past the top of the stock aperture into the otherwise unclaimed range below DRAM-high (`0x80_00000000`). The other four ports are never touched. An earlier design that enlarged the x8 window by borrowing address space from the other four ports — rebasing bridge 0 to `0x10_00000000` — was tested and fails outright: the card behind the x8 slot never comes up, and the resulting resource-allocation failure takes the integrated graphics down with it. The decode base of a root complex is fixed in silicon and cannot be relocated; growing bridge 0 upward from `0x18_00000000` does work and is confirmed good to at least `0x27_FFFFFFFF`.
- **The window is overridden at runtime rather than in the aperture table** — `mPcieResourceAppeture` lives in the prebuilt closed-source `PlatformPcieLib.lib` and cannot be edited. Disassembly of `PcieInitDxe.efi`, which links the same library and carries its own copy of the table, shows it reads only the `BusBase` and `BusLimit` fields (offsets +4 and +12) and never touches the memory-window fields or programs any outbound window from them. `PciHostBridgeLib` can therefore substitute its own above-4G window when constructing each root bridge without desyncing the closed binary.
- **Kept the prefetchable above-4G window aliased to the resized one** — the stock aperture table points `PMemAbove4G` at the same range as `MemAbove4G`, and PciBusDxe draws a GPU's large BAR from the prefetchable pool. Both now track the selected window, so the resize reaches the allocation that actually matters. Collapsing the two pools instead would have left prefetchable 64-bit BARs with nowhere to go, since the root bridges do not advertise `EFI_PCI_HOST_BRIDGE_COMBINE_MEM_PMEM`.

#### ACPI
- **Report the programmed PCIe window in `_CRS` instead of a hardcoded 16 GB** — Linux resizes BARs against the host bridge window it reads from `_CRS`, not against anything UEFI decided, so a resized window that ACPI still described as 16 GB would have changed nothing for the OS. `AcpiSocDxe` now publishes each root bridge's base and size (in 4 GB units, one byte each) into the ACPI RAM variable area at the new `ARV_PCIE_RP_0*_MEM64_BASE/SIZE_OFFSET` slots, and the `_CRS` methods of `PCI0`–`PCI4` in `Dsdt-Pcie.asl` patch their 64-bit `QWordMemory` descriptor from those values via `CreateQWordField`. The vendor Cadence root complexes `PRC0`–`PRC4` in `Dsdt-CdnsPcie.asl` do the same from their `_INI` methods, so both the mainline and vendor-kernel views agree with the firmware.

#### Device tree
- **Rewrite the PCIe 64-bit `ranges` entry for device-tree boots** — `UpdatePcieMem64Ranges()` in `DtbUpdateLibSi` patches the 64-bit memory entry (space code 3) of each root complex's `ranges` property to the window the firmware programmed, leaving the config and 32-bit memory entries untouched. Without this the toggle would have had no effect when booting with a device tree instead of ACPI. Node paths for the three remaining root complexes were added alongside the existing x8 and x4 definitions.

#### Setup variables
- **Guard the new setup field against a shorter stored variable** — `PLATFORM_SETUP_DATA` gained a `PcieBarWindow` byte at the end. A `PlatformSetupVar` written by an earlier build stops short of it, and `GetVariable` succeeds with the shorter size, so the field would otherwise read as uninitialized stack. `PcieWindowLib` zeroes the struct first and additionally checks that the returned size actually reaches the field before trusting it, falling back to `Auto` if not. The variable self-heals at full length the first time settings are saved in setup.
